...
const NavLink = ({title, description, url, icon }: { title: string, description: string, url: string, icon: IconDefinition }) => {
  ...
  <>
          <li>
            <Link href={url} className="flex items-center p-3 text-base  rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 font-semibold dark:text-white">
  ...
  return (
    <>
      ...
      <div className="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:p-6 dark:bg-gray-800 dark:border-gray-700">
        <p className="text-md text-gray-500 font-semibold dark:text-gray-500">Information for observation and rescuing within the Yarra Bend Flying Fox colony.</p>
